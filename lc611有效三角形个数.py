# Title     : lc611有效三角形个数.py
# Created by: julse@qq.com
# Created on: 2021/8/4 9:04
# des : 611. 有效三角形的个数
...
def solve1(nums):
    nums = filter(lambda ns:check(list(ns)),combinations(nums,3))
...
'''
长度为3的子集总数 - 不能组合成三角形的子集数目
https://blog.csdn.net/bianxia123456/article/details/105151104
power() 速度快于**
逆元+快速幂 速度快于 直接阶乘
list(nums) 和 [x for x in nums] 谁快？
...
#     m = 3
#     if n<m:return 0
#     cnm =1 if m==n else reduce(lambda x,y:x*y,range(1,n+1))/reduce(lambda x,y:x*y,range(1,m+1))/reduce(lambda x,y:x*y,range(1,n-m+1))
#     nums.sort()
#     count = 0
...
print([x for x in combinations(nums,3)])
# ans1 = solve1(nums)
# ans = solve2(nums)
# ans2 = solve3(nums)
# ans2 = solve4(nums)